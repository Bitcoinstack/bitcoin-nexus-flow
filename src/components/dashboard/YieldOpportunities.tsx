  ...
  return (
    <div className="bg-black/40 backdrop-blur-md p-6 rounded-xl border border-white/10 flex flex-col h-full">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-bold">Yield Opportunities</h3>
      ...
      <div className="space-y-4">
        {opportunities.length > 0 ? (
          opportunities.map((option) => (
            <div key={option.id} className="bg-black/50 rounded-lg p-4 border border-white/5 hover:border-bitcoin/30 transition-all">
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="font-medium text-white">{option.name}</h4>
                  <p className="text-sm text-gray-400">Provider: {option.provider}</p>
                </div>
